...
describe('API response schemas', () => {
  ...
  it('exposes shift assignees only through the assignment collection', () => {
    const shift = RosterShiftDetailSchema.parse({
      id: IDS.entity,
      rosterId: IDS.roster,
      personId: IDS.actor,
      startTime: NOW,
    ...
    expect(shift).not.toHaveProperty('personId');
    expect(shift.assignments.map((assignment) => assignment.personId)).toEqual([
      IDS.actor,
      IDS.period,
    ]);
  });
