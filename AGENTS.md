...
> **Status:** These commands are placeholders. They will be implemented as the repo matures.

| Command | What It Does | Status |
|---|---|---|
| `make setup` | Install dependencies, start Docker services, seed test data | 🔜 Planned |
| `make check` | Full validation: lint + type-check + schema-validate + tests | 🔜 Planned |
| `make lint` | Run linter in check mode (no auto-fix) | 🔜 Planned |
| `make lint-fix` | Auto-fix lint + formatting issues | 🔜 Planned |
| `make typecheck` | TypeScript compiler in `--noEmit` mode | 🔜 Planned |
| `make schemas` | Validate all JSON Schemas compile correctly | 🔜 Planned |
| `make generate` | Generate TypeScript types from schemas + OpenAPI | 🔜 Planned |
| `make test-unit` | Run unit tests only (target: <10s) | 🔜 Planned |
| `make test-integration` | Run integration tests (requires Docker) | 🔜 Planned |
| `make test-acceptance` | Run acceptance tests (full stack) | 🔜 Planned |
| `make test-compliance` | Run GDPR/audit compliance tests | 🔜 Planned |
| `make test-all` | Run all test suites | 🔜 Planned |
| `make docs` | Generate documentation site from markdown | 🔜 Planned |
| `make dev` | Start development server with hot reload | 🔜 Planned |
| `make clean` | Remove build artifacts, stop Docker, prune volumes | 🔜 Planned |
